...
<script>
    ...
    function addEvent(event) {
        ...
        Array.from(tbody.rows).forEach(function(row) {
            // Extract start and end time from the row's time slot
            var rowTime = row.cells[0].textContent.split(' - ');
            var rowStartTime = rowTime[0];
            var rowEndTime = rowTime[1];

            // Check if the event overlaps with the row's time range
            if (startTime < rowEndTime && endTime > rowStartTime) {
                ...
                for (var i = 1; i < row.cells.length; i++) {
                    ...
                    if (daysOfWeek[i - 1] === day) {
                        ...
                        var rowStartMinutes = timeToMinutes(rowStartTime);
                        var rowEndMinutes = timeToMinutes(rowEndTime);
                        eventDiv.style.top = ((startMinutes - rowStartMinutes) / (rowEndMinutes - rowStartMinutes) * 100) + '%';
                        eventDiv.style.bottom = ((rowEndMinutes - endMinutes) / (rowEndMinutes - rowStartMinutes) * 100) + '%';
